...
double Chro::fx()	//	适应度函数fx
{
	...
	for (int i = 0; i < gene_sum-1; i++){
		dis += distance(gene[i], gene[i + 1], gene[i+gene_sum], gene[(i+1+gene_sum)]);
	}
	fits = q_const / dis;// 总距离越大，适应度越小
	return fits;
} 
void Chro::display()	// 展示基因序列和该染色体适应度
{
	...
	cout << " |+|";
	for (int i = gene_sum; i < 2*gene_sum; i++) {
		cout << " " << gene[i];
	}
	cout << " ],";
	*/
	if(abs(fits)<1e-5)
		cout << "\tfits= " << fits <<",\tdistance= "<<q_const/fx();
	else
		cout << "\tfits= " << fx()<<",\tdistance= "<<q_const/fx();
}
void Chro::copy(Chro s)	//复制
{
	for (int i = 0; i < 2*gene_sum; i++){
		gene[i] = s.gene[i];
	}
	fits = fx();	// 重新计算适应度
}
